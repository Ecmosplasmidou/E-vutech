...
const ParticipantSelector = ({ participants, setParticipants, maxParticipants = 60 }) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-8 border border-slate-100 shadow-sm">
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4 text-center md:text-left">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Un minimum de 4 participants complets est requis.</p>
        </div>
        ...
        <div className="mb-6 p-4 bg-orange-50 border border-orange-100 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
          ...
          <div className="space-y-1 text-left">
            ...
            <p className="text-xs text-orange-700 leading-relaxed">
              Veuillez ajouter au moins <span className="font-bold">{MIN_REQUIRED} participants</span> complets. 
              Il en manque <span className="font-bold">{MIN_REQUIRED - totalValidParticipants}</span>.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar mb-6">
        {participants.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-slate-100 rounded-[2rem] bg-slate-50/50">
            ...
            <p className="text-slate-400 text-sm italic">Ajoutez votre premier participant.</p>
          </div>
        ) : (
          participants.map((p, index) => {
            const emailError = p.email && !isValidEmail(p.email);
            const isEntryValid = p.nom && p.prenom && isValidEmail(p.email) && isValidTel(p.tel) && isValidPDC(p.PDC);
                ...
                <div className={`flex-grow grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 p-5 rounded-[1.5rem] border transition-all ${
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] uppercase font-black text-slate-400 ml-1 flex justify-between">
                      Email * {emailError && <span className="text-red-500 lowercase font-medium italic text-[8px]">Invalide</span>}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-white border rounded-xl pl-9 pr-3 py-2 text-sm outline-none transition-all ${
                          emailError ? 'border-red-200 ring-4 ring-red-50' : 'border-slate-100 focus:ring-2 ring-primary/20'
                        }`}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] uppercase font-black text-slate-400 ml-1 flex justify-between">
                      Tél * {telError && <span className="text-red-500 lowercase font-medium italic text-[8px]">Invalide</span>}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-white border rounded-xl pl-9 pr-3 py-2 text-sm outline-none transition-all ${
                          telError ? 'border-red-200 ring-4 ring-red-50' : 'border-slate-100 focus:ring-2 ring-primary/20'
                        }`}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] uppercase font-black text-slate-400 ml-1 flex justify-between">
                      Permis * {PDCError && <span className="text-red-500 lowercase font-medium italic text-[8px]">Invalide</span>}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-white border rounded-xl pl-9 pr-3 py-2 text-sm outline-none transition-all ${
                          PDCError ? 'border-red-200 ring-4 ring-red-50' : 'border-slate-100 focus:ring-2 ring-primary/20'
                        }`}
                      />
                    </div>
                  </div>
      ...
      <div className="space-y-3">
        <button
          ...
          className={`w-full flex items-center justify-center gap-3 py-4 md:py-5 border-2 border-dashed rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all group ${
        ...
        {isLastParticipantInvalid && (
          <p className="flex items-center justify-center gap-2 text-[9px] md:text-[10px] font-bold text-orange-400 uppercase tracking-widest animate-pulse text-center">
            <AlertCircle size={12} /> Complétez la ligne actuelle pour continuer
          </p>
        )}
      </div>
    </div>
